...
public class AWSExecutor {
    ...
    private static final String FS_S3_PATH_STYLE_ACCESS = "fs.s3.path.style.access";

    private static final String AP_EAST_1 = "policy";
    ...
    private static final String PATH_SCHEMA_S3A = "s3a";
...
//    private static final String FS_S3A_FAST_UPLOAD = "fs.s3a.fast.upload";
    ...
    public AWSExecutor(String accessKey,String secretKey,String endPoint,String singleRegion){
        if (StrUtil.isAllNotBlank(accessKey,secretKey,endPoint,singleRegion)){
            ClientConfiguration clientConfig = new ClientConfiguration();
            AWSStaticCredentialsProvider credentials = new AWSStaticCredentialsProvider(new BasicAWSCredentials(accessKey,secretKey));
            this.s3Client = AmazonS3ClientBuilder.standard()
                    .withClientConfiguration(clientConfig)
                    .withCredentials(credentials)
                    .withEndpointConfiguration(new AwsClientBuilder.EndpointConfiguration(endPoint,singleRegion))
                    .withPathStyleAccessEnabled(true)
                    .build();
            conf.set(FS_S3A_ACCESS_KEY, accessKey);
            conf.set(FS_S3A_SECRET_KEY, secretKey);
            conf.set(FS_S3A_FAST_UPLOAD, Boolean.toString(true));
            conf.set(FS_S3A_PATH_STYLE_ACCESS, Boolean.toString(true));

//            conf.set("fs.s3a.access.key", "M60N1GQ9E4KBV9R1UN00");
//            conf.set("fs.s3a.secret.key", "Zbz0DOpFQ5oVSKQfwvhl6hGbDQKPtkYBFaDbaEeh");
            conf.set("fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem");
            conf.set("fs.s3a.endpoint", endPoint);
            conf.set("fs.s3a.connection.ssl.enabled", String.valueOf(false));
//            conf.set("fs.s3a.path.style.access", String.valueOf(true));
//            conf.set("fs.s3a.fast.upload", String.valueOf(true));
        }else {
            throw new SdkClientException("Unable to load AWS credentials from environment v ariables " + "(" + ACCESS_KEY_ENV_VAR + " and " + SECRET_KEY_ENV_VAR +  ")");
        }
    }
...
//        return new Path(PATH_SCHEMA, bucket, StrPool.SLASH + prefix);
        return new Path(PATH_SCHEMA_S3A, bucket, StrPool.SLASH + prefix);
    }
